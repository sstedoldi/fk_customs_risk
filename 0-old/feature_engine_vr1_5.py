...
class Feature_engine(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, tariff_code='HS06', 
        ...
        self.stats_vars = ['Unitprice', 'WUnitprice']#, 'Coeficiente del Concepto SUM'] to add it, feature selection should be reexecuted
        # tariff code stats
        ...
        self.code_activity = {}
        self.activity_vars = ['Codigo Aduana de registro - DDT',
                              'Pais de origen - ART',
                              'Pais de Procedencia - ART']
    ...
    def basic_features(self, X):
        X['NCM'] = X['Posicion Aranc.SIM-NCE - ART'].str.replace('.','').str[:8]
        X[self.tariff_code] = X['Posicion Aranc.SIM-NCE - ART'].str.replace('.','').str[:6]
        X.loc[:,'Unitprice'] = X.loc[:,'Monto Fob en U$S - ART SUM'] / X.loc[:,'Cantidad Estadistica - ART SUM']
        X.loc[:,'WUnitprice'] = X.loc[:,'Monto Fob en U$S - ART SUM'] / X.loc[:,'Peso Neto en Kilos - ART SUM']
        X['Origen != Procedencia'] = np.where(X['Pais de origen - ART'] != X['Pais de Procedencia - ART'], True, False)
    ...
    def date_features(self, X):
        X['day_of_week'] = X['Fecha de Oficializacion - DDT'].dt.dayofweek
    ...
    def fit(self, X=None, y=None):
        ...
        if self.stats_and_act_work == "read":
            ...
            if (not self.code_activity.empty) & self.save_act:
    ...
    def transform(self, X, y=None):
        if self.fitted or self.stats_work == 'read':
