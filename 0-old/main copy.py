...
## pre-process
import pandas as pd
# from local_prepro import DDT_cleaner
## transformations
...
def predict():
    try:
        ...
        print(X_request)
        print(pd.DataFrame(X_request, index=[0]))
        print(type(X_request))
        print('------------------------')
        print('------------------------')
        print('# New request #')
        ...
        print(X)
        # import checking
        # if len(X['importacion-exportacion(IMPEXP)']=='I') == len(X):
        if True:
            X_ = X.copy()
            # cleaning and preprocess
            # X_ = cleaner.preprocess(X_)
            # feature engine transformation
            X_ = feature_engine.transform(X_)
            # one-hot encoding
            # X_ = pd.get_dummies(data=X_, columns=cate_to_oneHotEncoding)
            # for var in onehot_feature_list: #completing columns not present
            #     if var not in X_.columns:
            #         X_[var]=False
            # log transformation
            X_, log_vars = num_to_log(X_, to_log)
            # var selection
            # var_selected = feature_selected + onehot_feature_list + log_vars
        ...
        # else:
        #     response_data = {'error': 'Import operation required'}
